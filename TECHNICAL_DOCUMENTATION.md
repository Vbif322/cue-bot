...
- `groupPhaseService.getGroupFramePoints(tournamentId)` — суммы очков по фреймам (`match_frames`) на каждый матч группового этапа; матч отсутствует в карте, если пофреймовой разбивки нет
...
- порядок тай-брейков в группе: победы → личные встречи среди равных → разница фреймов → **разница очков** → выигранные фреймы → сид
- разница очков участвует в сортировке только при `GroupStanding.pointsComplete` — когда у всех сыгранных матчей группы есть фреймы. Структурные walkover'ы (пустой слот) исключение: они дают 0 очков всем поровну. Матч без фреймов между реальными игроками (техпоражение, агрегатный ввод счёта, админская правка — она удаляет фреймы) закрывает ворота для всей группы, и таблица ведёт себя ровно как до M2-8. Тот же флаг управляет показом колонки «Очки» в боте, админке и игровом вебе
