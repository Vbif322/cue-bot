...
- **M2-10 / P1 — РЕШЕНО (закрыто, 2026-07-14):** детальный счёт для не-снукера
  (пул/русский) **не нужен**. Для пула и русского фиксируем только финальный счёт по
  партиям через пресет-пикер (`matchCommands.ts:623-658`); пофреймовая детализация
  (`match_frames`) остаётся снукер-онли. Второй миграции (`player1Points`/`player2Points`
  → nullable) не будет — колонки `match_frames` остаются `NOT NULL`. Ветка `isSnooker()`
  (`matchCommands.ts:81-84`) и пресет-пикер сохраняются как есть.
