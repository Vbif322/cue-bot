...
export async function handleDateInput(
  ...
  await ctx.reply(`Дата: ${formatDate(parsedDate)}\n\nШаг 3/${STEPS_COUNT}: Выберите площадку:`, {
...
export async function handleTableSelectionDone(ctx: BotContext): Promise<void> {
  ...
  await ctx.answerCallbackQuery();
  await finalizeTournamentCreation(ctx, state, state.data.selectedTableIds ?? []);
}
...
async function renderTablesStep(
  ...
  const availableTables =
    tables ?? (state.data.venueId ? await getTablesByVenue(state.data.venueId) : []);
