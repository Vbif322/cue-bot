...
- Удаление турниров
- Подтверждение/отклонение заявок участников
- Добавление/удаление участников
- Ручное изменение результатов матчей
- Дисквалификация участников
- Полный контроль над ходом турнира: управление порядком матчей, назначение столов, изменение очереди
- Переключение режима управления столами (авто/ручной)
...
- Просмотр детальной информации о турнире:
  ...
  - Статус регистрации
  - Место проведения (площадка/клуб)
  - Список используемых столов (при наличии)
...
- Указание площадки (клуб, зал)
- Назначение столов для турнира (один или несколько)
- Настройка параметров двойной элиминации (при выборе данного формата):
  - Включение/выключение перемешивания участников после каждого раунда
  - Выбор раунда, на котором нижняя сетка (losers bracket) переходит в верхнюю (winners bracket) для финальной части
- Включение обязательного подтверждения регистрации администратором
- Публикация турнира
...
- Получение уведомления о том, что заявка принята и ожидает подтверждения администратора (если включена модерация)
- Получение уведомления о подтверждении или отклонении заявки администратором
- Попадание в лист ожидания, если достигнут лимит участников:
  - Участник уведомляется о своей позиции в листе ожидания
  - При переводе администратором в основной список участник получает уведомление о подтверждении участия
- Отмена регистрации (до начала турнира)

#### 4.2.2 Управление участниками (Администратор)

- Просмотр списка зарегистрированных участников (подтвержденных и ожидающих подтверждения)
- Просмотр листа ожидания с позициями участников
- Подтверждение или отклонение заявок на участие (если включена модерация)
- Перевод участника из листа ожидания в основной список вручную (при освобождении места или решении администратора)
- Ручное добавление участников
- Удаление участников из основного списка
...
- Для двойной элиминации с перемешиванием: участники перераспределяются в новые пары в начале каждого раунда (в рамках своей сетки — верхней или нижней)
- Для двойной элиминации: переход участников нижней сетки в верхнюю происходит на заданном администратором раунде
...
- Заявка принята, ожидает подтверждения администратора
- Регистрация подтверждена / отклонена администратором
- Попадание в лист ожидания (с указанием позиции)
- Перевод из листа ожидания в основной список участников
- Формирование турнирной сетки
- Приближение времени матча (за 1 час, за 30 минут) с указанием стола
...
- Новая заявка на участие (требует подтверждения, если включена модерация)
- Достижение лимита участников
- Стол освободился и требует назначения следующего матча (в ручном режиме)
- Спорные ситуации с результатами
- Долгое отсутствие результата матча

### 4.6 Модуль "Площадки и столы"

#### 4.6.1 Управление площадками (Администратор)

- Создание площадки (название, адрес, описание)
- Добавление столов к площадке (название/номер стола, тип)
- Редактирование и удаление площадок и столов
- Привязка турнира к площадке

#### 4.6.2 Назначение столов на матчи и управление очередью

Турнир может работать в одном из двух режимов управления столами:

**Автоматический режим:**

- При освобождении стола система автоматически назначает на него следующий матч из очереди
- Очередность матчей определяется автоматически (по раунду и порядку в сетке)
- Участники получают уведомление с номером стола сразу после автоматического назначения

**Ручной режим:**

- Администратор видит текущий статус каждого стола (свободен / занят, идущий матч)
- Администратор вручную назначает матч на освободившийся стол из очереди ожидающих матчей
- Администратор управляет очередью матчей: может менять их порядок
- Администратор может переназначить матч на другой стол в любой момент
- Участники получают уведомление с номером стола после ручного назначения

**Общие требования для обоих режимов:**

- Принцип максимальной занятости столов: все назначенные для турнира столы должны быть заняты в любой момент, пока есть ожидающие матчи
- Система отображает очередь ожидающих матчей и статус каждого стола в реальном времени
- Отображение назначенного стола в информации о матче
- Администратор может переключаться между режимами

### 4.7 Модуль "Дисквалификация"

#### 4.7.1 Дисквалификация участника (Администратор)
...
**Двойная элиминация** — формат турнира с двумя сетками: верхней (winners bracket) и нижней (losers bracket); участник выбывает только после двух поражений

**Верхняя сетка (winners bracket)** — основная сетка двойной элиминации, куда попадают участники без поражений

**Нижняя сетка (losers bracket)** — дополнительная сетка двойной элиминации для участников с одним поражением

**Перемешивание участников** — режим двойной элиминации, при котором участники перераспределяются в новые пары в начале каждого раунда внутри своей сетки

**Круговая система** — формат, где каждый играет с каждым

**Протокол матча** — запись результата игры

**Площадка** — место проведения турнира (бильярдный клуб, зал)

**Стол** — конкретный бильярдный стол на площадке, назначаемый для проведения матча

**Лист ожидания** — список участников, подавших заявку на турнир после достижения лимита мест; администратор вручную выбирает, кого включить в основной список при освобождении места
